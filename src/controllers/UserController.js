...
class UserController {
  async store(req, res) {
    try {
      const novoUser = await User.create({ // Cria um novo User
        nome: req.body.nome,
  ...
  async show(req, res) {
    try {
      ...
      const user = await User.findByPk(id); // Busca um User pelo ID
  ...
  async update(req, res) {
    try {
      const user = await User.findByPk(req.userId); // Busca um User pelo ID
      if (!user) {
        return res.status(400).json({
